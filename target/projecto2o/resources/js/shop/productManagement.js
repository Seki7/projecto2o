...
function getProduct() {
    var url = "/shop/getProductList?pageIndex="+pageIndex+"&pageSize="+pageSize+"&productName="+productName+"&productCategoryId="+productCategoryId;
    $.ajax({
...
function handleProductList(data) {
    ...
    data.map(function(item,index) {
        html +=  '<div class="sui-row"> <div class="span1">' + item.productId +'</div> <div class="span1">'
            ...
            '</div> <botton onclick="goDetailPage('+item.productId+')" class="sui-btn btn-bordered btn-success" >管理</botton></div>';
    });
...
function goDetailPage(i) {
    window.location.href= "/shop/productDetail?"+"productId="+i;
